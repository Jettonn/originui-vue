...
const maxCount = computed(() => Math.max(...itemCounts.value))
...
const isBarInSelectedRange = (index: number) => {
  ...
  return (
    countItemsInRange(sliderValue.value[0], sliderValue.value[1]) > 0 &&
    rangeMin <= sliderValue.value[1] &&
    rangeMax >= sliderValue.value[0]
  )
}
</script>
